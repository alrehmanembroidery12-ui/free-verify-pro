    ...
    res.send(`
        ...
        <body class="flex flex-col items-center justify-center min-h-screen p-6 text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl w-full">
                <div onclick="location.href='/student'" class="glass p-12 rounded-[3rem] cursor-pointer hover:border-blue-500/50 transition-all group">
                    <div class="text-6xl mb-4 group-hover:scale-110 transition">🎓</div>
                    <h2 class="text-xl font-bold uppercase tracking-widest">Student Portal</h2>
                </div>
                <div onclick="location.href='/login'" class="glass p-12 rounded-[3rem] cursor-pointer hover:border-emerald-500/50 transition-all border-emerald-500/10 group">
                    <div class="text-6xl mb-4 group-hover:scale-110 transition">🔐</div>
...
app.get('/login', (req, res) => {
    res.send(`
        ...
        <body class="flex flex-col items-center justify-center min-h-screen p-6">
            <a href="/" class="text-2xl font-black text-blue-500 mb-10 italic">FEEVERIFY PRO</a>
            <div class="glass p-10 rounded-[3rem] border border-white/10 w-full max-w-md glow-blue">
                <h2 class="text-xl font-black mb-8 uppercase text-center italic tracking-widest">Authentication</h2>
                <div class="space-y-4">
                    ...
                    <button onclick="doLogin()" class="w-full bg-blue-600 p-5 rounded-2xl font-black uppercase tracking-widest">Continue</button>
                </div>
            </div>
            <script>
                async function doLogin() {
                    ...
                    if(res.status === 200) { location.href = data.is2FA ? '/two-factor' : '/admin-panel?auth=true'; }
...
app.get('/two-factor', (req, res) => {
    res.send(`
        ...
        <body class="flex flex-col items-center justify-center min-h-screen p-6 text-center">
            ...
            <h2 class="text-2xl font-black mb-2 italic uppercase">Mobile Security</h2>
            <div class="glass p-8 rounded-[2.5rem] glow-blue max-w-sm w-full">
                <button onclick="location.href='/admin-panel?auth=true'" class="w-full bg-emerald-600 p-5 rounded-2xl font-black shadow-lg">APPROVE ACCESS</button>
            </div>
        </body>
    `);
});
    ...
    res.send(`
        ...
        <body class="flex h-screen overflow-hidden">
            <aside class="hidden md:flex flex-col w-72 glass border-r border-white/5 p-6 z-50">
                <a href="/" class="text-2xl font-black italic text-blue-500 mb-12">FEEVERIFY <span class="text-white">PRO</span></a>
                ...
                <button onclick="location.href='/'" class="w-full p-4 rounded-2xl bg-red-500/10 text-red-500 font-bold text-xs">Logout</button>
            </aside>
            <main class="flex-1 overflow-y-auto p-4 md:p-12 pb-32">
                <div id="main-dash" class="main-tab">
                    <div class="flex justify-between items-end mb-10">
                        <h1 class="text-4xl font-black italic">Live Monitor</h1>
                        <div class="glass px-8 py-4 rounded-[2rem] border-emerald-500/20 text-right">
                            <p class="text-2xl font-black text-emerald-400">Rs ${total.toLocaleString()}</p>
                        </div>
                    </div>
                            ...
                            <tbody class="divide-y divide-white/5">
                                ${txs?.map(t => `
                                    <tr>
                                        ...
                                        <td class="p-6 text-right font-black">Rs ${t.amount}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>
                ...
                <div id="main-set" class="main-tab hidden max-w-3xl mx-auto">
                    ...
                    <div class="flex justify-center gap-4 mb-10">
                        <button onclick="switchSubSet('db')" id="sub-btn-db" class="sub-tab-btn active px-6 py-3 rounded-full border border-white/10 text-[10px] font-black uppercase transition-all">🗄️ Database</button>
                        <button onclick="switchSubSet('acc')" id="sub-btn-acc" class="sub-tab-btn px-6 py-3 rounded-full border border-white/10 text-[10px] font-black uppercase transition-all">👤 Account</button>
                    </div>
                    <div id="subset-db" class="subset-box glass p-10 rounded-[3rem] border-blue-500/20">
                        <input type="text" id="sUrl" value="${config.supabaseUrl}" class="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-xs mb-4" placeholder="URL">
                        <input type="password" id="sKey" value="${config.supabaseKey}" class="w-full bg-white/5 border border-white/10 p-5 rounded-2xl text-xs mb-4" placeholder="Key">
                        <button onclick="saveAll()" class="w-full bg-blue-600 p-5 rounded-2xl font-black text-xs uppercase">Sync</button>
                    </div>
                </div>
            </main>
            <script>
